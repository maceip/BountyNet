...
android {
    ...
    defaultConfig {
        ...
        versionCode = 1
        versionName = "0.1.0"
        // Must match a deployed `/android-auth` page (see clients/web/src/pages/AndroidAuth.tsx). Override in debug below for local Vite.
        ...
        // Optional: ship Timber batches to gateway `POST /logs/android` (set BOUNTYNET_CLIENT_LOG_TOKEN on server).
        // Example: "\"https://gateway.example.com/logs/android\"" and matching bearer in LOG_SHIP_TOKEN.
...
val patchKeyAttestation by tasks.registering(Exec::class) {
    group = "bountynet"
    // Repo root is two levels above the Gradle root (…/clients/android).
    workingDir = rootProject.rootDir.parentFile.parentFile
