...
impl Status {
    pub fn run(self) -> Result<()> {
        ...
        info!("agent #{}", data.agent_id.unwrap_or(config.agent_id));
        info!("  wallet: {}", data.wallet.unwrap_or(config.wallet));
        info!("  ens:    {}", data.ens.unwrap_or(config.ens.clone()));

        if let Some(bal) = data.balances {
            info!("  eurc:   {} EURC", bal.eurc.unwrap_or_default());
            info!("  native: {}", bal.native.unwrap_or_default());
        }

        if let Some(rep) = data.reputation {
            info!(
                "  solved: {} bounties, {} EURC earned",
