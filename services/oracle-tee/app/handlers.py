"""
BountyNet CI Oracle — TEE / signing handlers.

Receives CI proof requests, signs them with the attestation-held key.
The signature can be verified on any EVM chain via ecrecover.

Operations:
  ORACLE/VALIDATE — Sign a CI pass/fail proof
  ORACLE/ATTEST   — Sign a GitHub OIDC attestation hash

Relayed flow (when a framework dispatches to this process):
  on-chain dispatch → handler signs → result posted back per deployment wiring.

Direct HTTP path:
  Gateway may POST /oracle/sign to this service.
"""
...
def register(framework: Framework) -> None:
    """Register the ORACLE handlers with the host framework."""
...
def sign_ci_proof_direct(repo: str, sha: str, check_name: str, conclusion: str) -> dict:
    ...
    relayed on-chain dispatch. Same key, same signature, faster path.
