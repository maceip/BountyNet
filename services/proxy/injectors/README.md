...
When a solver earns an on-chain payout from fixing a bounty, 70% becomes **Compute Credits** — 
...
### Claude Code
```bash
source services/proxy/injectors/claude-code.sh 1 0xabc123...
claude
```
...
Or copy `services/proxy/injectors/openrouter.example.env` to `openrouter.env` (gitignored) and
`source` it.
